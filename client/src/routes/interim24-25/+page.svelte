  ...
  onMount(() => {
    ...
    if (isMobile) {
      rawClasses = rawClasses.filter(function (el, index, self) {
        return (
          index === self.findIndex((t) => t[0] === el[0] && t[1] === el[1])
        );
      });
      filter();
    }
  ...
  function filterSearch(term) {
    ...
    classes = classes.filter(function (el) {
      ...
      if (
        ...
        if (
          el[0].toLowerCase().includes(words[0].toLowerCase()) &&
          el[1].toLowerCase().includes(words[1].toLowerCase())
        ) {
          found = true;
        }
        return found;
      ...
      words.forEach((word) => {
        if (
          el[0].toLowerCase().includes(word.toLowerCase()) ||
          el[3].toLowerCase().includes(word.toLowerCase()) ||
          el[1].toLowerCase().includes(word.toLowerCase())
        ) {
          found = true;
        }
      });
...
<div class="my-4 mx-2 font-medium text-center justify-items-center">
  ...
  <p>Showing Interim Winter 2024-25 Classes</p>
  <p></p>
</div>

<div class="flex flex-col items-center justify-center">
  <p
    class="bg-yellow-200 dark:bg-yellow-400 border-2 border-yellow-300 dark:border-yellow-500 dark:text-slate-900 inline-block p-2 max-w-screen rounded-lg text-center m-2"
  >
    Please be aware that the information on this page may be inacurate and
    incomplete.
    <br />Data last updated 10/24/2024
  </p>
</div>

<div
  class="m-2 flex flex-wrap justify-center items-center text-sm dark:text-slate-400"
>
  <!-- Add a row of checkboxes coresponding to the booleans above. -->
  <label
    class="inline-flex items-center bg-slate-400 dark:bg-slate-800 font-bold px-4 py-2 rounded-full m-1 cursor-pointer shadow-lg"
  >
...
<div class="flex justify-center">
  <div
    class="bg-slate-100 rounded-lg flex w-5/6 md:w-1/2 lg:w-1/3 shadow-lg dark:bg-slate-300"
  >
    <input
      class="border-none bg-transparent outline-none p-2 w-full placeholder-slate-500 dark:placeholder-slate-700 dark:text-slate-900"
      bind:value={search}
      on:input={() => filter()}
...
{#if numClasses == 0}
  <p class="font-bold m-6">
    No Classes Found. Try reducing the filters or clearing the search bar
  </p>
{/if}
...
{#each Array(numClasses).fill(0) as _, i}
  <div
    class="bg-slate-200 p-2 m-2 md:m-6 flex flex-wrap items-center rounded-lg shadow-md dark:bg-slate-700 dark:text-slate-100"
  >
    <p class="font-bold">{classes[i][0]} {classes[i][1]}&nbsp;</p>
    {#if !isMobile}
      <p class="font-medium">
        {classes[i][2]} - {classes[i][3]} - {classes[i][4]}&nbsp
      </p>
    {/if}
    {#if isMobile}
